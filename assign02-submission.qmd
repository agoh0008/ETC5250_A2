...
Overall, this implies that the misclassifications made by the two models may not be consistently occurring for the same data points.
...
We should also use interactive graphics to further explore the misclassifications stemming from both the linear discriminant analysis and logictic regression models. The detour plot provides deeper insights into class distribution and potential misclassification areas, while the confusion matrix scatterplots highlight differences in model performance and misclassification patterns.
...
Comparing the scatterplots of the confusion matrices between the two models also highlights disparities in their performance. In @fig-6, the LDA model demonstrates a higher number of misclassifications for both `finance` and `birdsongs` classes compared to logistic regression. 

However, when looking at the positions and distributions of misclassified observations within the plots, there appears to be some similarities between the models. For instance, if we highlight the misclassified `finance` data points in the top left corner for both @fig-6 (LDA model) and @fig-7 (logistic model), we can see that the misclassified observations are approximately in the same area as they appear to be attached to the border of the `birdsongs` cluster (i.e. at the curved area of the `birdsongs` cluster). 

<br>


![](images/lda_finance.png)


<br>


![](images/log_finance.png)


Similarly, highlighting misclassified `birdsongs` data points in the bottom right corner of both the LDA (@fig-6) and logistic regression models' (@fig-7) plots reveals a common trend. These misclassified observations are clustered near the misclassified `finance` points at the border of the `birdsongs` cluster, along with the presence of one distinct misclassified `birdsongs` outlier. 

These observations suggest shared challenges faced by both models, including potential class overlap and limitations in capturing complex class boundaries. The outlier's misclassification is understandable, given its deviation from typical class characteristics. However, the placement of misclassified `birdsongs` observations at the border of the `birdsongs` cluster raises questions about the models' ability to accurately distinguish subtle differences within complex class distributions. This misclassification of `birdsongs` points at the border of the `birdsongs` cluster itself indicates difficulties in accurately delineating class boundaries, especially in regions where class distributions overlap. 

<br>


![](images/lda_birdsongs.png)


<br>


![](images/log_birdsongs.png)



Therefore, contrary to our previous findings from @fig-4 and @fig-5, the models do seem to be making mistakes for the same observations. However, it's still consistent with our previous analysis that the LDA model demonstrates a higher number of misclassifications for both `finance` and `birdsongs` classes compared to logistic regression. Specifically, LDA falsely predicts 17 `finance` observations as `birdsongs` and 15 `birdsongs` observations as `finance` across both the training and test sets. In contrast, as seen in @fig-7, logistic regression has fewer misclassifications, with only 6 `finance` observations misclassified as `birdsongs` and 11 `birdsongs` observations misclassified as `finance`.

In summary, although the logistic model appears to exhibit higher prediction accuracy compared to LDA, notable similarities in their class distributions and potential misclassification areas become apparent upon closer examination through interactive graphics. 
